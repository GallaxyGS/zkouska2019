<?php include_once __DIR__ . DIRECTORY_SEPARATOR . "inc" . DIRECTORY_SEPARATOR . 'header.php' ; ?>
<?php $id_player = filter_input(INPUT_GET, "id_player"); ?>
<?php function vek($ymd) {
    list($rok,$mesic,$den) = explode("-",$ymd);
    $y = Date("Y")-$rok;
    $t = strtotime("$rok.$mesic.$den") - strtotime(Date("$rok.m.d"));
    return $t > 0 ? $y - 1 : $y;
} ?>
<!DOCTYPE html>
<html lang="cs" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title></title>
  </head>
  <body>
<div class="table-responsive">
        <table class="table table-striped table-sm">
                  <thead>
                  <tr>
                    <th>ID Hráče</th>
                    <th>Jméno</th>
                    <th>Přijmení</th>
                    <th>Věk</th>
                    <th>Počet vstřelených gólů</th>
                  </tr>
                </thead>
                <tbody>
                  <?php
                  $sql = $mysqli->prepare("SELECT * FROM players WHERE id_player=?");
                  $sql->bind_param("d", $id_player);
                  $sql->execute();
                  $result = $sql->get_result();
                  
                         while ($players = $result->fetch_assoc()) {
                       ?>

                      <tr>
                       <td> <?php echo $players['id_player'];  ?> </td>
                       <td> <?php echo $players['firstname'];  ?> </td>
                       <td> <?php echo $players['lastname'];  ?> </td>
                       <td> <?php echo vek($players['birthdate']);?>  </td>
                       <td> <?php echo $players['goal_number'];  ?> </td>
                      <?php

                     }

                      ?> </tr>
                </tbody>
              </table>
            </div>
        </body>
      </html>



<?php include_once __DIR__ . DIRECTORY_SEPARATOR . "inc" . DIRECTORY_SEPARATOR .  'footer.php' ?></body>
